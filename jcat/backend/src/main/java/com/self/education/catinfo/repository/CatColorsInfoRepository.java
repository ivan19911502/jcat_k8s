...
public interface CatColorsInfoRepository extends JpaRepository<CatColorsInfo, Colors> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO cat_colors_info (cat_color, count) VALUES (CAST(:color AS TEXT), :count)", nativeQuery = true)
    void save(String color, int count);

    @Query(value = "SELECT * FROM cat_colors_info WHERE CAST(cat_color AS TEXT) = :color", nativeQuery = true)
    Optional<CatColorsInfo> findByColor(String color);

    @Modifying
    @Transactional
    @Query(value = "UPDATE cat_colors_info SET count = :count WHERE CAST(cat_color AS TEXT) = :color", nativeQuery = true)
